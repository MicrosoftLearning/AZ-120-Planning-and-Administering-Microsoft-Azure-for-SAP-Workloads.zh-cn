...
1. 在“**创建邻近放置组**”边栏选项卡的“**基本信息**”选项卡中，请指定以下设置并选择“**查看 + 创建**”（保留其他设置的默认值）：
...
1. 在“创建虚拟机”边栏选项卡的“基本信息”选项卡中，指定以下设置并选择“下一步:************ 磁盘 >”（保留所有其他设置的默认值）：
    ...
    |   --    |  --   |
    | **订阅** | Azure 订阅的名称 |
    | **资源组** | 之前在此任务中创建的资源组的名称 |
    | **虚拟机名称** | **az12001a-vm0** |
    | **区域** | 在创建邻近放置组时选择的 Azure 区域的名称 |
    | **可用性选项** | **可用性集** |
    | **可用性集** | 名为 **az12001a-avset** 的新可用性集，具有 2 个容错域和 5 个更新域 |
    | 安全类型**** | **标准** |
    | **图像** | **SUSE Enterprise Linux for SAP 15 SP5 - BYOS - x64 Gen 2** |
    ...
    > **备注**：要查找映像，请单击“**查看所有映像**”链接，在“**选择映像**”边栏选项卡上的搜索文本框中，键入“**SUSE Enterprise Linux**”，然后在结果列表中单击“**SUSE Enterprise Linux for SAP 15 SP5 - BYOS**”并选择“**Generation 2**”。
...
1. 在“**创建虚拟机**”边栏选项卡的“**网络**”选项卡上，在“**网络接口**”部分，直接在“**虚拟网络**”文本框下方，选择“**新建**”。 
1. 在“**创建虚拟网络**”窗格中，指定以下设置，然后选择“**确定**”：
...
1. 返回“**创建虚拟机**”边栏选项卡的“**网络**”选项卡上，指定以下设置并选择“**下一步：管理 >**”（保留所有其他设置的默认值）：
...
1.  在“**创建虚拟机**”边栏选项卡的“**高级**”选项卡中，确保配置以下设置并选择“**查看 + 创建**”（保留其他所有设置的默认值）：
...
7. 重复上一步，使用前缀“az12001a-vm1-DataDisk”附加剩余的 7 个磁盘（总共 8 个）。**** 分配与磁盘名称的最后一个字符匹配的 LUN 编号。 将 LUN \ **1** 磁盘的主机缓存设置为“**只读**”，并将其余所有磁盘的主机缓存设置为“**无**”。
...
1. 在“**az12001a-vm0**”边栏选项卡中，选择“**连接**”，在下拉菜单中选择“**通过 Bastion 连接**”，在“**az12001a-vm0**”的“**Bastion**”选项卡上，将“**身份验证类型**”保留为“**VM 密码**”，提供部署“**az12001a-vm0**”虚拟机时设置的凭据，保留启用的复选框“**在新浏览器选项卡中打开**”，然后选择“**连接**”。
...
1. 通过运行以下命令在 vi 编辑器中打开 **/etc/fstab**（也可以使用任何其他编辑器）：
   ...
   > **备注**：如果使用 vi 编辑器，请按 **i** 键进入“插入”模式。

   ![vi 编辑器“插入”模式指示器](../media/az120-lab01-vieditor-insert.png)
...
1. 通过运行以下命令验证装载是否成功：
   ...
   ![df-h 输出](../media/az120-lab01-df-output.png)
1. 通过运行以下命令退出特权模式：
...
1. 在与 **az12001a-vm1** 的 Bastion 会话中，通过运行以下命令生成无密码 SSH 密钥对：
...
1. 通过运行以下命令，将新生成的密钥对的公钥复制到 **az12001a-vm0**：

      ```sh
   ssh-copy-id -i /home/student/.ssh/id_rsa.pub student@az12001a-vm0
   ```

1. 当系统提示确认是否要继续连接时，请输入“**是**”，然后按 **Enter** 键。

1. 当系统提示进行身份验证时，请输入之前在此实验室预配 **az12001a-vm0** 时设置的密码。

1. 切换到与 **az12001a-vm0** Azure VM 的 Bastion 会话。

1. 在与 **az12001a-vm0** 的 Bastion 会话中，通过运行以下命令生成无密码 SSH 密钥对：
...
1. 通过运行以下命令，将新生成的密钥对的公钥复制到 **az12001a-vm1**：
...
1. 当系统提示进行身份验证时，请输入之前在此实验室预配 **az12001a-vm1** 时设置的密码。

1. 要验证配置是否成功，请在与 **az12001a-vm0** Azure VM 的 Bastion 会话中，通过运行以下命令，以**学生**身份建立与 **az12001a-vm1** 的 SSH 会话： 
...
1. 切换到与 **az12001a-vm1** Azure VM 的 Bastion 会话。

1. 在与 **az12001a-vm1** 的 Bastion 会话中，通过运行以下命令，以**学生**身份建立与 **az12001a-vm0**的 SSH 会话： 
